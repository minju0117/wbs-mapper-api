...
⚠️ 퍼블리싱 마감 준수 (절대 규칙):
- 단일 오픈: 모든 퍼블리싱은 운영배포일 최소 10영업일 전 완료.
- 1차/2차 오픈: 1차 메뉴 퍼블리싱은 모두 first_launch_date 최소 5영업일 전 완료. 2차 메뉴 퍼블리싱은 모두 end_date 최소 7영업일 전 완료.
- 기간이 빠듯하면 각 메뉴 퍼블 duration을 최솟값(2~3일)으로 줄여서라도 준수.

### STEP 8. 검수 및 개발

**단일 오픈:**
| Front | 내부검수 | 퍼블리싱 검수 및 수정 | 전체 메뉴의 절반 이상 퍼블 완료 후 | 7~18 | 더위버 |
...
**1차/2차 오픈 검수:**
1차 퍼블 검수 시작 = 1차 메뉴 전체의 절반 이상 퍼블 완료 시점 (전체 완료 전에 시작 가능)
2차 퍼블 검수 시작 = 2차 메뉴 전체의 절반 이상 퍼블 완료 시점

### STEP 9. 오픈

⚠️ 역산 원칙: 오픈일에서 역산하여 일정 배치. 일정이 빠듯하면 앞 단계 duration을 줄임.

**단일 오픈 (first_launch_date 없음):**
운영배포(end_date) ← 통합테스트(3~5일) ← 고객사검수 완료 순으로 역산.
...
[역산] 1차 오픈(=first_launch_date) ← 1차 서버포팅(1일) ← 통합테스트(3일) ← 1차 퍼블 검수(7~10일)
→ 1차 메뉴 퍼블리싱은 모두 "통합테스트 시작일" 이전에 완료되어야 함.

[역산] 2차 오픈(≈end_date) ← 2차 서버포팅(1일) ← 2차 개발(5일) ← 2차 퍼블 검수(7~8일)
→ 2차 메뉴 퍼블리싱은 모두 "2차 퍼블 검수 종료일" 이전에 완료되어야 함.
