"""
WBS 매퍼 FastAPI 서버
POST /map-wbs  — download WBS + template WBS 업로드 → 완성된 엑셀 반환
"""
...
async def map_wbs(
    ...
    template: UploadFile = File(..., description="원본 WBS 템플릿 xlsx"),
    sheet: str = Query(default="V1.4", description="템플릿 시트 이름"),
):
    for f in (download, template):
        if not f.filename.endswith(".xlsx"):
            raise HTTPException(status_code=400, detail=f"{f.filename}: xlsx 파일만 허용됩니다.")
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        try:
            ...
            raise HTTPException(status_code=422, detail=f"템플릿 파일 오류: {e}")
